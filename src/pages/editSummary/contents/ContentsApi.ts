...
import { fetchPosts, albumPosts, type Post } from "@/api/api";
...
export const useStatusAlbums = (
  options?: Omit<UseQueryOptions<Post[]>, "queryKey" | "queryFn">
) => {
  const query = useQuery({
    queryKey: ["status-albums"],
    queryFn: albumPosts,
    staleTime: 60000,
    gcTime: 300000,
    ...options,
  ...
  // 필요한 필드만 추출하여 변환된 데이터
  const albums: StatusPost[] = query.data
    ? query.data.map((post) => ({
        id: post.id,
        userId: post.userId,
        title: post.title,
